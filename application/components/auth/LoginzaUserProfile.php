...
class LoginzaUserProfile {
	...
	public function genNickname () {
		if (!empty($this->profile->nickname)) {
			return $this->normalize($this->profile->nickname,'_');
		} elseif (!empty($this->profile->email) && preg_match('/^(.+)\@/i', $this->profile->email, $nickname)) {
			return $this->normalize($nickname[1],'_');
		} elseif ( ($fullname = $this->genFullName()) ) {
			return $this->normalize($fullname, '_');
		}
		...
		foreach ($patterns as $pattern) {
			if (preg_match('/^https?\:\/\/'.$pattern.'/i', $this->profile->identity, $result)) {
				return $this->normalize($result[1],'_');
			}
		}
	...
	public function genUserSite () {
        ...
        if (preg_match('/^https?\:\/\/www.google.com/i', $this->profile->identity, $result)) {
            return '';
        }
	...
	public function genFullName () {
		if (!empty($this->profile->name->full_name)) {
			...
			return trim((!empty($this->profile->name->first_name)?$this->profile->name->first_name:'').' '.(!empty($this->profile->name->last_name)?$this->profile->name->last_name:''));
		}
